...
public class AddressTableUpdater {
    ...
    public void handleContactCreatedEvent(ContactCreatedEvent event) {
    	...
    	newEntry.setIdentifier(event.getContactIdentifier());
    ...
    public void handleContactNameChangedEvent(ContactNameChangedEvent event) {
		...
		ContactEntry contact = gtx.get(ContactEntry.CONTACT, contactKey);
		contact.setName(newName);
    	
    	List<Key> addressKeys = Datastore.query(AddressEntry.META)
    								.filter(AddressEntry.META.identifier.equal(contactId))
    								.asKeyList();

    	if (addressKeys.size() > 0) {
    		List<AddressEntry> addressList = gtx.get(AddressEntry.META, addressKeys);
    ...
    public void handleContactDeletedEvent(ContactDeletedEvent event) {
    	...
    	List<Key> addressKeys = Datastore.query(AddressEntry.META)
		.filter(AddressEntry.META.identifier.equal(contactId)).asKeyList();
    	if (addressKeys.size() > 0) {
    		session.delete(addressKeys);
    	}
    	
    	AggregateIdentifier aggregateId = event.getAggregateIdentifier();
    	session.delete(Datastore.createKey(ContactEntry.CONTACT, aggregateId.asString()));
    }
    ...
    public void handleAddressDeletedEvent(AddressRemovedEvent event) {
    	...
    	List<Key> keys = Datastore.query(AddressEntry.META)
    		.filter(AddressEntry.META.identifier.equal(event.getContactIdentifier()))
    		.filter(AddressEntry.META.addressType.equal(event.getType())).asKeyList();
    ...
    public void handleAddressChangedEvent(AddressChangedEvent event) {
    	...
    	AddressEntry a = Datastore.query(AddressEntry.META)
			.filter(AddressEntry.META.identifier.equal(event.getContactIdentifier()))
			.filter(AddressEntry.META.addressType.equal(event.getType())).asSingle();
    	
    	AddressEntry target = gtx.get(AddressEntry.META, a.getKey());
    ...
    public void handleAddressAddedEvent(AddressAddedEvent event) {
		...
		Key contactKey = ContactEntry.createKey(event.getContactIdentifier());
    	ContactEntry contact = gtx.get(ContactEntry.CONTACT, contactKey);
    	
    	AddressEntry entry = new AddressEntry();
    	entry.setKey(AddressEntry.createKey());
